...
(() => {
  ...
  const tabs = {};
  let persistTabsTimeoutId = null;

  function persistTabsState() {
    if (persistTabsTimeoutId) {
      clearTimeout(persistTabsTimeoutId);
    }
    persistTabsTimeoutId = setTimeout(() => {
      persistTabsTimeoutId = null;
  ...
  function injectMain(messageType, nameAttribute) {
    ...
    function addMediaFromNode(node) {
      ...
      node.querySelectorAll?.('video:not([global-media-controls]), audio:not([global-media-controls])').forEach((video) => {
    ...
    observeDOM(document, (mutations) => {
      mutations.forEach((mutation) => {
        mutation.addedNodes.forEach((node) => addMediaFromNode(node));
      });
    });
  }
  ...
  async function createPanelCustom(panel, webviewButton) {
    if (!chrome.extension.inIncognitoContext) {
      ...
      let [showCloseButton, autoClose, asOverlayEnabled] = await Promise.all([
        vivaldi.prefs.get('vivaldi.panels.show_close_button'),
        vivaldi.prefs.get('vivaldi.panels.as_overlay.auto_close'),
        vivaldi.prefs.get('vivaldi.panels.as_overlay.enabled'),
      ]);
      ...
      const buttonClose = gnoh.createElement('button', {
        ...
        style: {
          display: showCloseButton.value && asOverlayEnabled.value && autoClose.value || !showCloseButton.value ? 'none' : 'flex',
        },
        events: {
          click() {
      ...
      vivaldi.prefs.onChanged.addListener((path, value) => {
        ...
        buttonClose.style.display = showCloseButton.value && asOverlayEnabled.value && autoClose.value || !showCloseButton.value ? 'none' : 'flex';
      });
  ...
  const style = !chrome.extension.inIncognitoContext ? [
    ...
    'button[data-name="' + webPanelId + '"]:before { width: 16px; height: 16px; content: ""; background-color: var(--colorFg); -webkit-mask-box-image: url(' + JSON.stringify(icons.dataURLs.playlistMusic) + '); }',
  ...
  function createWebPanel() {
    vivaldi.prefs.get('vivaldi.panels.web.elements', (elements) => {
      // Support both old API (direct array) and new API ({value: [...]})
      const elementsArr = (elements && elements.value !== undefined) ? elements.value : elements;
      let element = elementsArr.find((e) => e.id === webPanelId);
      if (!element) {
        element = {
          activeUrl: code,
      ...
      Promise.all(
        ...
        const hasGlobalMediaControl = toolbars.some((toolbar) => {
          ...
          return arr.some((p) => p === webPanelId);
        });
        ...
        if (!hasGlobalMediaControl) {
          ...
          const panelIndex = panelsArr.findIndex(panel => panel.startsWith('WEBPANEL_'));
