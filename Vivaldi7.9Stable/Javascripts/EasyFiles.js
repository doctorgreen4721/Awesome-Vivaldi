...
(() => {
  ...
  function inject(nameKey) {
    ...
    function handleClick(event) {
      if (event.target.matches("input[type=file]:not([webkitdirectory])")) {
        ...
        elementClickedRect.bottom =
          elementClickedRect.bottom - pointerPosition.y;
  ...
  async function getDownloadedFiles(accept) {
    ...
    for (let downloadedFile of downloadedFiles) {
      if (
        ...
        downloadedFile = (
          await chrome.downloads.search({ id: downloadedFile.id })
        )[0];
  ...
  function createFileIcon(extension) {
    ...
    const colorBgLighter = gnoh.color.shadeColor(
      colorBg.r,
      colorBg.g,
      colorBg.b,
      isLightBg ? 0.4 : -0.4
    ...
    const fileIconContent = gnoh.createElement(
      "div",
      {
  ...
  async function createSelectbox(sender, file, dialog) {
    ...
    const selectboxImage = gnoh.createElement(
      "div",
      {
    ...
    if (file.previewUrl) {
      const image = gnoh.createElement(
        "img",
        {
          src: file.previewUrl,
        },
      ...
      const extension =
        file.extension || gnoh.file.getFileExtension(file.fileName);
      selectboxImage.append(createFileIcon(extension));
    }

    const selectboxTitle = gnoh.createElement(
      "div",
      {
    ...
    if (file.fileName) {
      ...
      const filenameContainer = gnoh.createElement(
        "div",
        {
          class: "filename-text",
          text: name,
      ...
      if (extension) {
        const filenameExtension = gnoh.createElement(
          "div",
          {
  ...
  async function showDialogChooseFile({
    info,
    sender,
    clipboardFiles,
    downloadedFiles,
    ...
    const buttonShowAllFilesElement = gnoh.object.merge(
    ...
    function setPosition(entries) {
      for (const entry of entries) {
        ...
        if (info.elementClickedRect.left < 0) {
          ...
          info.elementClickedRect.left + dialogRect.width >
          window.innerWidth
        ) {
          dialog.modalBg.style.left =
            Math.max(info.elementClickedRect.right - dialogRect.width, 0) +
            "px";
        } else {
        ...
        if (info.elementClickedRect.bottom < 0) {
          ...
          info.elementClickedRect.bottom + dialogRect.height >
          window.innerHeight
        ) {
          dialog.modalBg.style.top =
            Math.max(info.elementClickedRect.top - dialogRect.height, 0) + "px";
        } else {
    ...
    if (clipboardFiles.length) {
      ...
      const h3Clipboard = gnoh.createElement(
        "h3",
        {
    ...
    if (downloadedFiles.length) {
      ...
      const h3Downloaded = gnoh.createElement(
        "h3",
        {
      ...
      switch (info.action) {
        case "click":
          ...
          if (clipboardFiles.length || downloadedFiles.length) {
            ...
            info.elementClickedRect.left =
              info.elementClickedRect.left * ratio + pointerPosition.x;
            info.elementClickedRect.top =
              info.elementClickedRect.top * ratio + pointerPosition.y;
            info.elementClickedRect.right =
              info.elementClickedRect.right * ratio + pointerPosition.x;
            info.elementClickedRect.bottom =
              info.elementClickedRect.bottom * ratio + pointerPosition.y;
            info.elementClickedRect.width =
              info.elementClickedRect.width * ratio;
            info.elementClickedRect.height =
              info.elementClickedRect.height * ratio;
            info.elementClickedRect.x =
              info.elementClickedRect.x * ratio + pointerPosition.x;
            info.elementClickedRect.y =
              info.elementClickedRect.y * ratio + pointerPosition.y;
